...
//                }
    ...
    private func getSessionId(_ completionHandlerForToken: @escaping (_ success: Bool, _ requestToken: String?, _ errorString: String?) -> Void) {
        
        /* 1. Specify parameters, method (if has {key}), and HTTP body (if POST) */
        /* 2. Make the request */
        
        let jsonBody = "{\(ParameterKeys.Udacity): {\(ParameterKeys.Username): }}"
        
        let _ = taskForPOSTMethod(Methods.Session, parameters: nil, ) { (results, error) in
